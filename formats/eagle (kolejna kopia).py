...
class EaglePCB(mainPCB):
    ...
    def generate(self, doc, groupBRD, filename):
        ...
        for i in range(self.dialogMAIN.spisWarstw.rowCount()):
            if self.dialogMAIN.spisWarstw.cellWidget(i, 0).isChecked():
                ...
                if layerNumber == 47:  # MEASURES
                ...
                elif layerNumber in [17, 18]:  # pady
                    self.getPads(doc, layerNumber, grp, name, color, transp)
                    ...
                    self.generateConstraintAreas(self.getConstraintAreas(layerNumber), doc, ID, grp_2, name, color, transp)
                else:
                    pass
    ...
    def getPads(self, doc, layerNumber, grp, layerName, layerColor, defHeight):
        ...
        layerSide = softLayers["eagle"][layerNumber]['side']
        #layerType = PCBlayers[softLayers["eagle"][layerNumber][1]][3]
        layerType = softLayers["eagle"][layerNumber]['type']
        ####
